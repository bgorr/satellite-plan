...
for row_key in rows.keys():
    ...
    initial_reobs = []
    replan_reobs = []
    for i in range(int(np.max([np.max(replan_observations),np.max(initial_observations)]))):
    ...
    plt.savefig(plot_dir+"/"+row_key+"_lininc_linear_082024.png",dpi=300, bbox_inches="tight")
