...
param max_slew_rate := 1*3.14159/180; # rad/s
param max_slew_time := 3.14159/3/max_slew_rate; # rad/(rad/s)
...
param access[TIPS] :=
	read "/home/ben/repos/satplan/src/utils/accesses_2h_5sat_processed.csv" as "<11n,10n,9n,8n> 12n" skip 1 default 0;

# slewing constraint parameters
param times[T] := 
	read "/home/ben/repos/satplan/src/utils/accesses_2h_5sat_processed.csv" as "<11n> 5n" skip 1 default 0;

# set reward values
param r[I] := 
	read "/home/ben/repos/satplan/src/utils/accesses_2h_5sat_processed.csv" as "<10n> 7n" skip 1 default 0;
...
subto slew_torque: forall <s,t,k> in ST2K with k<t and (times[t] - times[k] <= max_slew_time) do 
	vif sum <p> in P : o[t,p,s] == 1 and sum <p> in P : o[k,p,s] == 1 then
		(abs((sum <p> in P : o[t,p,s]*theta[p]) - (sum <p> in P : o[k,p,s]*theta[p]))/((times[t] - times[k]))) <= max_slew_rate end, indicator;
